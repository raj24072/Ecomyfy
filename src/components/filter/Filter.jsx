...
function Filter() {
  ...
  const handleReset = () => {
    ...
    ...new Set(getProduct ? getProduct.map((item) => item.category) : []),
  ];

  // Extract unique prices
  const uniquePrices = [
    ...new Set(getProduct ? getProduct.map((item) => item.price) : []),
  ];
  return (
    <div>
        ...
        >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="px-8 w-full py-3 rounded-md bg-violet-0 border-transparent outline-0 text-sm "
              style={{
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
              <select
               ...
               onChange={e => setFilterType(e.target.value)}
                className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0 focus:border-gray-500 focus:bg-white ocus:ring-0 text-sm"
                style={{
                  backgroundColor: mode === "dark" ? "rgb(64 66 70)" : "",
                  color: mode === "dark" ? "white" : "",
                }}
              ...
              >
                ...
                {getProduct && getProduct.map((item)=> {
                  return(
                    <option key={item.id} value={item.price}>{formatNumber(item.price)}</option>
                  )
                })}
              </select>
            </div>
